<template>
    <tig-layout @loginSuccess="loginSuccess">
        <template v-if="!showLogin">
            <tig-navbar :showLeft="false" title="购物车"></tig-navbar>
            <view class="cart-box shoppingCart" v-if="!isLoad">
                        ...
                        <template v-for="(item, index) in cartList" :key="item.cartId">
                            <view class="cart_table">
                                <view class="cart_store_title noborder">
                                    <tig-checkbox v-model:checked="item.isChecked" size="20" @change="onCheckAllItem(index)"></tig-checkbox>
                                    <view @click="handleToShop(item.shopId)" class="store_label">{{ item.shopTitle ? item.shopTitle : $t("自营") }}</view>
                                </view>
                                                    ...
                                                    <view class="cart_list_con" :class="{ cartItemDisabled: goods.isDisabled }">
                                                        ...
                                                        >
                                                            <view class="photo-img">
                                                                <image :src="imageFormat(goods.picThumb)"></image>
                                                                <view class="cart-notice-row" v-if="goods.stock === 0">{{ $t("已售罄") }}</view>
                                                                <view class="cart-notice-row" v-if="goods.productStatus === 0">{{ $t("已下架") }}</view>
                                                            </view>
                                                        </navigator>
                                                        <view class="cart-row">
                                                            <navigator
                                                            ...
                                                            <template v-if="goods.activityInfo">
                                                                <view class="activity-box">
                                                                    ...
                                                                    >
                                                                        <activityCard :activityInfo="subItem" cardType="sub"></activityCard>
                                                                    </template>
                                                                </view>
                                                            </template>
                                                            ...
                                                            <template v-if="goods.price && goods.originPrice !== goods.price">
                                                                <view class="origin-price">
                                                                    {{ $t("每件预计到手") }}
                                                                    <format-price :showText="false" :isBottom="false" :priceData="goods.price"></format-price>
                                                                </view>
                                                            </template>
                                                                            ...
                                                                            <view class="extraskudata-item">
                                                                                ...
                                                                                <view class="content">
                                                                                    ...
                                                                                    <format-price
                                                                                        :showText="false"
                                                                                        class="price"
                                                                                        :priceData="attr.totalAttrPrice"
                                                                                    ></format-price>
                                                                                    <text class="iconfont-h5 icon-xiajiantou1-copy"></text>
                                                                                </view>
                                                                            </view>
                                                                        </template>
                                                                    </template>
                                                                                ...
                                                                                <template v-if="isOverseas()">
                                                                                    <text class="line1 content-text"
                                                                                        ...
                                                                                        <format-price
                                                                                            :showText="false"
                                                                                            class="price"
                                                                                            :priceData="goods.extraSkuData[0].attrPrice"
                                                                                        ></format-price>
                                                                                    </text>
                                                                                </template>
                                                                                <template v-else>
                                ...
                                <template v-if="item.gift.length > 0">
                                    <view class="gift-box">
                                        ...
                                        <view class="gift-content">
                                            <view class="gift-content-item" v-for="subItem in item.gift" :key="subItem.giftId">
                                                <view class="gift-item-left">
                                                    <image :src="imageFormat(subItem.picThumb)"></image>
                                                </view>
                    ...
                    <view class="checkOutBar" :style="checkOutBarStyle">
                        <view class="bar-check">
                            <view class="checkbox-pad">
                                <tig-checkbox size="20" v-model:checked="allChecked" @change="onCheckAll"></tig-checkbox>
                        ...
                        <view class="cart-total-box" v-if="!cartManage">
                            <view class="cart-total">
                                <view @click="handleCheckout" :class="'btn-checkout ' + (total.checkedCount == 0 ? 'unable_btn' : '')">{{ $t("去结算") }}</view>
                                <view class="item-total">
                                    <view class="item-total-amount">
                                        <text class="txt">{{ $t("合计") }}：</text>
                                        <format-price
                                            :showText="false"
                                            :priceData="total.discountAfter"
                                            :fontStyle="{ fontWeight: 'bold', fontSize: '30rpx' }"
                                            :currencyStyle="{
                                                fontSize: '22rpx',
                                                fontWeight: 'bold'
                                            }"
                                    ...
                                    <view class="item-total-desc">
                                        <view class=""
                                            >{{ $t("共优惠") }}：<format-price :showText="false" :isBottom="false" :priceData="total.discounts"></format-price
                                        ></view>
                                        <view class="discounts-btn" @click="handleShowDiscountInfo">
                                            {{ $t("优惠明细") }}
                                            <text class="iconfont-h5 icon-xiajiantou1-copy" :class="{ 'icon-active': showDiscountInfo }"></text>
                                        </view>
                                    </view>
                                </view>
                            </view>
                ...
                <recommend @callback="getCartList"></recommend>
            </view>
        </template>
        <template v-else>
            <toLogin></toLogin>
        ...
        <discountInfo :total="total" v-model="showDiscountInfo"></discountInfo>
        <detail v-model="showDetail" :detail="currentDetail"></detail>
    </tig-layout>
</template>
