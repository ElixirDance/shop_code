<template>
    <tig-popup v-model:show="showCoupon" :zIndex="999" :showClose="false" backgroundColor="#fff">
                    ...
                    <template v-if="false">
                        <view class="discount-box">
                            <view class="discount-title">
                                ...
                                <format-price
                                    :showText="false"
                                    :fontStyle="{ fontSize: '23rpx', fontWeight: '400', color: 'var(--general)' }"
                                    priceData="18"
                                ></format-price>
                                <text @click="handleShowOfferDetail"
                                    >，{{ $t("优惠明细") }} <text class="iconfont-h5 icon-xiajiantou1-copy" :class="{ rotate: showOfferDetail }"></text
                                ></text>
                            </view>
                            <view class="discount-content" v-if="showOfferDetail">
                                <view class="discount-item">
                                    <view class="discount-item-label">{{ $t("限时活动价格共计：") }}</view>
                                    <view class="discount-item-content"
                                        ><format-price :showText="false" :fontStyle="{ fontSize: '23rpx', fontWeight: '400' }" priceData="18"></format-price
                                    ></view>
                                </view>
                            </view>
                        </view>
                    ...
                    <template v-if="activityList.length > 0 || giftList.length > 0">
                        <view class="activity-box">
                            ...
                            <template v-if="activityList.length > 0">
                                <view class="activity-item" v-for="item in activityList" :key="item.promotionId">
                            ...
                            <template v-if="giftList.length > 0" v-for="giftItem in giftList" :key="giftItem.promotionId">
                                <view class="activity-item" v-for="item in giftItem.data.promotionDesc" :key="item.giftId">
                                    ...
                                    <view class="activity-product">
                                        ...
                                        <scroll-view class="scroll-x" scroll-x="true">
                                            <view class="activity-product-item" @click="handleLink(item.gift.productInfo.productId)">
                                                <tig-image :src="item.gift.productInfo.picThumb" class="gift-img"></tig-image>
                                                <view class="gift-price">
                                                    <format-price
                                                        :showText="false"
                                                        :fontStyle="{ fontSize: '18rpx', fontWeight: 'bold' }"
                                                        :isBottom="false"
                                                        :priceData="item.gift.productInfo.productPrice"
                                                    ></format-price>
                                                </view>
                                            </view>
                                        </scroll-view>
                        ...
                        <view class="coupon-box">
                            ...
                            <view v-for="item in couponList" :key="item.couponId" class="coupon-item">
                                <view class="left">
                                    ...
                                    <view class="col-3" :class="{ 'special-col': !item.couponDesc }" v-if="item.sendType === 1">
                                        {{ $t("有效期") }}：{{ item.useEndDate }}
                                    </view>
                                    <view class="col-3" :class="{ 'special-col': !item.couponDesc }" v-if="item.sendType === 0 && item.delayDay > 0">
                                ...
                                <view class="right">
                                    <view class="right-1">
                                        ...
                                        <template v-else>
                                            <format-price
                                                :showText="false"
                                                :fontStyle="{ fontSize: '48rpx', lineHeight: 1, fontWeight: '400' }"
                                                :currencyStyle="{ selfAlign: 'end', fontSize: '28rpx' }"
                                                :priceData="item.couponMoney"
                                            ></format-price>
                                        </template>
                                    </view>
                                    <template v-if="couponStatus(item.isReceive!, item.limitNum, item.receiveNum) !== '已领取'">
                                        <view class="right-2">
                                            <view
                                                ><tig-button :customStyle="{ height: '55rpx' }" @click="getCoupon(item)">
