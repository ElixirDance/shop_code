                        ...
                        <view class="notifycenter-item-left">
                            <view class="item-left-title">
                                <view class="item-left-title-info line2"
                                    ><view class="title-tip" v-if="item.isTop === 1">{{ $t("置顶") }}</view> {{ item.title }}</view
                                >
                            </view>
                            <view class="item-left-desc line2" v-if="item.describe">{{ item.describe }}</view>
                            <view class="item-left-time">{{ item.startTime }}</view>
                        </view>
                        <view class="notifycenter-item-right">
                            <tig-image :src="item.img"></tig-image>
                        </view>
                    </view>
                </block>
            </view>
        </view>
        <empty v-if="!loading && list.length === 0" :styles="{ width: '320rpx', height: '320rpx' }"></empty>
    </tig-layout>
</template>
