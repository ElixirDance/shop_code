<template>
    <tig-layout :needSafeTop="true">
        <view class="level">
            <view class="level-dashboard">
                <view class="container">
                    <view class="header">
                        <template v-if="member.avatar">
                            <tig-image class="avatar" :src="member.avatar"></tig-image>
                        </template>
                        <template v-else>
                            <image :src="staticResource('salesman/avatar.png')" class="avatar"></image>
                        </template>
                        ...
                        <view class="level-stepper-wrap less">
                            <template v-if="nextLevelData.name">
                                ...
                                <view class="level-stepper-wrap__line">
                                    <view class="line-inner progress" :style="`width: ${nextLevelGapData.totalPerc}%;`"></view>
                                </view>
                ...
                <view class="rights-and-interests" v-if="tuiGuangRate(salesmanData.level) > 0">
                    <view class="item">
                        <view class="title-icon tui"></view>
                        <view>
                ...
                <view class="level-dashboard__rate-text">
                    <up-icon name="info-circle" color="#999" size="12"></up-icon>
                    {{ $t("部分商品的佣金比例可能因商家设置而有所不同，请以商品中所显示的为准") }}
                </view>
            </view>
            <view class="level-schedule" v-if="nextLevelData">
                <view class="title">{{ $t("满足以下规则可升级为") }}{{ nextLevelData.name }}</view>
                <view class="level-schedule__status">
                    <view class="level-schedule-item" v-if="salesmanData.condition?.salesAmount.checked">
                        <view class="tag-wrap">
                            <view
                        ...
                        <view class="tag-desc"
                            ...
                            <block v-if="nextLevelGapData.salesAmountInfo.salesAmountReach === false">
                                (还差<view class="level-schedule-item__left">{{ nextLevelGapData.salesAmountInfo.salesAmountGap }}</view>
                                {{ currencyName }})
                            </block>
                        </view>
                    </view>
                    <view class="level-schedule-item" v-if="nextLevelData.condition?.salesInviteUsers.checked">
                        <view class="tag-wrap">
                            <view
                                class="level-schedule-item__tag van-tag van-tag--plain van-tag--default"
                    ...
                    <view class="level-schedule-item" v-if="nextLevelData.condition?.selfBuyAmount.checked">
                        <view class="tag-wrap">
                            <view
                                class="level-schedule-item__tag van-tag van-tag--plain van-tag--default"
            ...
            <view class="level-rules" v-if="levelRules">
                        ...
                        <view class="desc">
                            <view class="desc-item">
                                ...
                                <view v-if="rule.condition.salesAmount.checked"
                                    >{{ rule.condition.salesAmount.title || $t("推广金额") }} {{ rule.condition.salesAmount.value }}
                                    {{ rule.condition.salesAmount.unit || currencyName }}</view
                                >
                                <view v-if="rule.condition.salesInviteUsers.checked"
                                    >{{ rule.condition.salesInviteUsers.title || $t("发展客户数") }} {{ rule.condition.salesInviteUsers.value }}
                                    {{ rule.condition.salesInviteUsers.unit || $t("人") }}</view
                                >
                                <view v-if="rule.condition.selfBuyAmount.checked"
                                    >{{ rule.condition.selfBuyAmount.title || $t("自购金额") }} {{ rule.condition.selfBuyAmount.value }}
