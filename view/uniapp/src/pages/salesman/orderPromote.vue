<template>
    <tig-layout>
        <timeFilter v-model:params="filterParams" @change="getSalesmanOrderList"></timeFilter>
                ...
                <block v-for="item in list" :key="item.orderId">
                    <view class="orderpromote-list-item">
                        <view class="orderpromote-list-item-status">
                            <view class="orderpromote-list-item-name" v-if="item.userOrder && item.userOrder.consignee"
                        ...
                        <view class="orderpromote-list-item-product">
                            <view class="item-product-left">
                                <view class="item-product-left-img">
                                    <tig-image :src="item.userOrderItem.picThumb"></tig-image>
                                </view>
                            </view>
                        ...
                        <view class="orderpromote-list-item-orderamount">
                            {{ $t("实付金额：") }}<format-price :priceData="item.orderAmount"></format-price>
                        </view>
                        <view class="orderpromote-list-item-amount">
                            {{ $t("商品佣金：") }}<format-price :fontStyle="{ color: '#ff720d', 'font-weight': 700 }" :priceData="item.amount"></format-price>
                        </view>
