<template>
    <view>
        <tig-layout>
            <view class="salesman-pro-detail-imgs" v-if="detailData.pics">
                <view class="product-bg">
                    <swiper :indicator-dots="false" :circular="true" interval="3000" duration="500" @change="swiperChange">
                        <block v-for="(item, index) in detailData.pics" :key="index">
                            <swiper-item>
                                <image lazy-load @click="swiperImagePreview(index)" :src="imageFormat(item.picUrl || '')" class="slide-image" />
                            </swiper-item>
                        </block>
                    </swiper>
            ...
            <view class="salesman-pro-detail-info">
                <view class="salesman-pro-detail-price">
                    <view class="salesman-pro-detail-price-num">
                        <format-price :priceData="detailData.productPrice"></format-price>
                    </view>
                    <view class="salesman-pro-detail-price-tip" v-if="Number(detailData.finalCommissionPrice) > 0">
                        <image class="price-tip-img" :src="staticResource('salesman/priceTip.png')"></image>
                        <view class="salesman-pro-detail-price-tip-text">
                            {{ $t("预计可赚") }} <format-price :priceData="detailData.finalCommissionPrice"></format-price>
                        </view>
                    </view>
                </view>
                ...
                <view class="bottom-btns">
                    ...
                    <view class="bottom-btn special" @click="showShare = true">
                        <view class="bottom-btn-text">
                            <view> {{ $t("分享赚钱") }}</view>
                            <text class="bottom-btn-text-tip" v-if="Number(detailData.finalCommissionPrice) > 0"
                                >{{ $t("分享后预计可赚") }} <format-price :priceData="detailData.finalCommissionPrice"></format-price
                            ></text>
                        </view>
                    </view>
                </view>
            ...
            <share :currentData="detailData" v-model:showShareCard="showShare"></share>
        </tig-layout>
    </view>
</template>
