<template>
    <view class="poster-main">
        <image v-if="img" :src="img" class="img-box" :style="{ height: boxInfo.boxHeight + 'px' }" @longpress.stop="handleSave"></image>
    </view>
    <canvas class="canvas-content" :style="{ height: boxInfo.boxHeight + 'px' }" :id="id" :canvas-id="id"></canvas>
</template>
...
const createPoster = async () => {
    ...
    try {
        ...
        boxInfo.boxHeight = calculateProportionalHeight(currentPicRes.height, currentPicRes.width).height + 140;
        boxInfo.imgHeight = calculateProportionalHeight(currentPicRes.height, currentPicRes.width).height;
        boxInfo.imgWidth = calculateProportionalHeight(currentPicRes.height, currentPicRes.width).width;
            ...
            setTimeout(() => {
                context.draw(true, async () => {
                    ...
                    context.draw(true, async () => {
                        ...
                        context.fillText(props.currentData.productPrice, 20, y + 30);
                        // 不加延迟小程序会出现诡异问题
                        setTimeout(() => {
                            context.draw(true);
                            setTimeout(() => {
