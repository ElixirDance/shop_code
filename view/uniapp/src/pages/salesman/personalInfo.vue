<template>
    <tig-layout :needSafeTop="true" safeTopBgColor="#f5f5f5">
        <up-form labelPosition="left" :model="personalData" :rules="rules" ref="formRef">
            <view class="personal-title">{{ $t("个人信息") }}</view>
            <view class="personal-info">
                <up-form-item :label="$t('头像')" labelWidth="100" :borderBottom="true" ref="itemRef">
                    <view class="salesman-avatar">
                        <tig-image class="salesman-avatar__img salesman-avatar--round salesman-avatar__img--big" :src="personalData.avatar"></tig-image>
                    </view>
                </up-form-item>
                <up-form-item :label="$t('昵称')" labelWidth="100" prop="nickname" :borderBottom="true" ref="itemRef">
                    <up-input v-model="personalData.nickname" border="none" :placeholder="$t('请输入昵称')" fontSize="14"></up-input>
                </up-form-item>
                <!-- <up-form-item
                    label="名片名字"
                    labelWidth="100"
            ...
            <view class="personal-info">
                <up-form-item :label="$t('手机号')" labelWidth="100" prop="mobile" :borderBottom="true" ref="itemRef">
                    <up-input v-model="personalData.mobile" border="none" :placeholder="$t('请输入手机号')" fontSize="14"></up-input>
                </up-form-item>
                <!-- <up-form-item
                    label="微信二维码"
                    labelWidth="100"
...
const save = async () => {
    try {
        const result = await updateSalesmanPersonal({ ...personalData });
        if (result.message) {
