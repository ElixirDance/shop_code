<template>
    <tig-layout ref="layoutRef">
        ...
        <view class="qianggou-list">
            <view class="qianggou-item" v-for="(item, index) in seckillProductList" :key="index" @click="toDetail(item)">
                <view class="qianggou-img-box">
                    <image class="qianggou-img" :src="imageFormat(item.picThumb || '')" mode="aspectFill"></image>
                    <view class="qianggou-out" v-if="item.seckillStock === 0">
                ...
                <view class="qianggou-info">
                    ...
                    <view class="qianggou-money-box">
                        ...
                        <view class="qianggou-btn" :class="{ flex: item.seckillStock === 0, 'align-center': item.seckillStock > 0 }">
                            <view class="btn-txt" v-if="item.seckillStock === 0"> {{ $t("已抢完") }} </view>
        ...
        <loading-box color="#fff" v-model="isLoadMore" :page="filterParams.page" :length="seckillProductList.length"></loading-box>
    </tig-layout>
</template>
