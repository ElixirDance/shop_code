    ...
    <view>
        <tig-layout :title="navbarTitle">
                        ...
                        <view class="coupon-box">
                            <view v-for="item in couponList" class="coupon-item">
                                <view class="left">
                                    ...
                                    <view class="col-3" v-if="item.sendType === 1"> {{ $t("有效期") }}：{{ item.useEndDate }} </view>
                                    <view class="col-3" v-if="item.sendType === 0 && item.delayDay > 0">
                                        {{
                                            isOverseas()
                                ...
                                <view class="right">
                                    <view class="right-1">
                                        ...
                                        <block v-else>
                                            <format-price
                                                :showText="false"
                                                :currencyStyle="{ 'justify-content': 'flex-end', fontSize: '24rpx', 'padding-bottom': '8rpx' }"
                                                :isBottom="false"
                                                :fontStyle="{ fontSize: '48rpx' }"
                                ...
                                <view class="right">
                                    ...
                                    <view class="right-2">
                                        <view>
                                            ...
                                            <tig-button :disabled="true" :customStyle="{ height: '55rpx' }" v-if="item.status === 3">
                                                {{ $t("即将生效") }}
                                            </tig-button>
                                            <tig-button :disabled="true" :customStyle="{ height: '55rpx' }" v-if="item.status === 4">
                                                {{ $t("已使用") }}
                                            </tig-button>
                                            <tig-button :disabled="true" :customStyle="{ height: '55rpx' }" v-if="item.status === 5">
                                                {{ $t("已过期") }}
                                            </tig-button>
                                        </view>
                                    </view>
            ...
            <view v-if="!loading && couponList && couponList.length === 0" class="empty-box">
                <view class="pictrue">
                    <image lazy-load :src="staticResource('common/data_empty.png')"></image>
                </view>
...
const handleReceiveCoupon = async (item: CouponFilterResult) => {
    ...
    try {
        const result = await addCoupon({ couponId: item.couponId });
        uni.showToast({
            title: t("领取成功"),
            duration: 1500
        });
