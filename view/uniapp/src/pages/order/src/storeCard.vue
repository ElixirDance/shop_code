        ...
        <view class="store-card-item">
                    ...
                    <view class="content-item" @click="handleLink(`/pages/product/index?id=`, subItem.productId)">
                        ...
                        <view class="store-card-item-content-right">
                            ...
                            <template v-if="subItem.skuData.length > 0">
                                <view class="sku-card">
                                    <template v-for="skuItem in subItem.skuData" :key="skuItem">
                            ...
                            <template v-if="subItem.activityInfo && activityList(subItem.activityInfo).length > 0">
                                <view class="activity-box">
                                    <template v-for="activityItem in activityList(subItem.activityInfo)" :key="activityItem.promotionId">
                                        <activityCard fontSize="22rpx" :activityInfo="activityItem" cardType="sub"></activityCard>
                                    </template>
                                </view>
                            </template>
                    ...
                    <view class="gift-content">
                        <view class="gift-content-item" v-for="subItem in item.gift" :key="subItem.giftId">
                            <view class="gift-item-left">
                                <tig-image :src="subItem.picThumb" class="gift-img"></tig-image>
                            </view>
            ...
            <view class="payment">
                ...
                <view class="payment-content" @click="handleDistributionMode(item.shopId, item.noShipping)">
                    ...
                    <image lazy-load class="more-ico" :src="staticResource('common/more.png')"></image>
                </view>
            </view>
        </view>
    </view>
    ...
    <tig-popup v-model:show="show">
        <view class="payment-popup">
            ...
            <view class="payment-btn">
                <template v-for="(item, index) in shippingTypeList[shippingTypeKey]" :key="item.shippingTypeId">
                    <view
                        class="payment-btn-item"
                        :class="{ active: getActive(index, item.shippingTypeId) }"
                        @click="handleShippingType(index, item.shippingTypeId)"
                        >{{ item?.shippingTypeName }}</view
                    >
                </template>
            </view>
            <view class="payment-desc">
                <view class="payment-desc-title">{{ currentShippingType?.shippingTypeName }}</view>
                <view class="payment-desc-text"> {{ currentShippingType?.shippingTimeDesc }} </view>
            </view>
...
const currentShippingType = computed(() => {
    return props.shippingTypeList[shippingTypeKey.value][currentIndex.value];
});
const getShippingTypeText = (id: number, noShipping: number) => {
    return noShipping === 1 ? "无需配送" : (props.shippingType && props.shippingType[id] && props.shippingType[id].typeName) || t("该收货地址不支持配送");
};
