<template>
    <view class="coupon-info">
                ...
                <view class="points">
                    <view class="points-title"
                        ...
                        <view class="points-text"
                            >{{ $t("当前账户积分") }}：<text class="special-text">{{ points }}</text>
                        </view>
                    </view>
                    <view class="points-content" @click="handlePoints">
                        <view class="points-text" v-if="pointsAmount > 0">{{ $t("共抵扣：") }}{{ currency + pointsAmount }}</view>
                        <view class="points-text" v-else-if="points === 0 && pointsAmount === 0">{{ $t("暂无可用积分") }}</view>
                        <view class="points-text" v-else>{{ isOverseas() ? $t("最多可用{0}积分", [availablePoints]) : `最多可用${availablePoints}积分` }}</view>
                        <image lazy-load class="more-ico" :src="staticResource('common/more.png')"></image>
                    </view>
                </view>
            </template>
        ...
        <template v-if="configStore.useSurplus == 1">
            <view class="balance">
                ...
                <view>
                    <switch :checked="isBalance" color="var(--general)" @change="handleBalance" style="transform: scale(0.8)" />
                </view>
            </view>
        </template>
    </view>
    <tig-popup v-model:show="show" backgroundColor="#fff">
            ...
            <view class="coupon-container-content">
                <template v-if="tabsActive === '可用优惠券'">
                    <view class="coupon-box">
                        <view v-for="(item, idx) in couponListData!.enableCoupons" :key="idx" class="coupon-item">
                            <view class="left">
                                <view class="col-1">
                            ...
                            <view class="right">
                                ...
                                <view class="right-2">
                                    <tig-checkbox v-model:checked="item.selected" @change="handleCheck(item)"></tig-checkbox>
                                </view>
                            </view>
                    ...
                    <view class="coupon-box">
                        <view v-for="(item, idx) in couponListData!.disableCoupons" :key="idx" class="coupon-item grayScaleDiv">
                            ...
                            <view class="right">
                                ...
                                <view class="right-2">
                                    <tig-checkbox :disabled="true" v-model:checked="item.selected"></tig-checkbox>
                                </view>
                            </view>
                            <view class="dotted-line"></view>
                        </view>
    ...
    <tig-popup v-model:show="showPoints" backgroundColor="#f5f5f5">
