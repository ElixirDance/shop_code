        ...
        <view class="order">
            <view class="order-menu">
                <template v-for="item in menuList" :key="item.type">
                    <view class="order-menu-item" :class="{ active: currentType === item.type }"
                        @click="handleMenuType(item.type)">
                        <uni-badge v-if="item.num > 0" class="badgecolor" :text="item.num" absolute="rightTop"
                            size="small">
                    ...
                    <template v-for="item in filterState" :key="item.orderId">
                        <view class="order-list-item">
                            <view class="order-list-item-header">
                                <text class="special-text">{{ $t("订单编号") }}：<text class="special-color">{{ item.orderSn
                                }}</text>
                                </text>
                            </view>
                                                ...
                                                <view>
                                                    <view class="item-content-product-info">
                                                        ...
                                                        <view class="product-item-price">
                                                            <format-price :showText="false" :decimalsStyle="{
                                                                fontSize: '25rpx'
                                                            }" :currencyStyle="{
                                                            fontSize: '23rpx'
                                                        }" class="product-item-pricenum"
                                                                :priceData="subItem.price"></format-price>
                                                            <view class="product-item-quantity">x {{ subItem.quantity }}
                                                            </view>
                                                        </view>
                                                    </view>
                                                    <view class="sku-card"
                                                        :class="{ nodata: !subItem.skuData || subItem.skuData.length === 0 }">
                                                        <template v-if="subItem.skuData && subItem.skuData.length > 0">
                                                            <view v-for="(skuItem, skuIndex) in subItem.skuData"
                                                                :key="skuIndex" class="sku-item line1">{{
                                                                    skuItem.value
                                                                }}</view>
                                                        </template>
                                                    </view>
                            ...
                            <view class="order-list-item-btn">
                                <view class="item-btn-box">
                                    <tig-button :plain="true" :customStyle="{ height: '55rpx' }" :plainMainColor="true"
            ...
            <template v-else>
                <view class="comment-list">
                    <view class="comment-item flex" v-for="item in filterState" :key="item.orderId">
                        <view class="comment-header">
                            <tig-image class="head" v-if="item.picThumb" :src="item.picThumb"
                                mode="aspectFill"></tig-image>
                        </view>
                        <view class="comment-info">
                            <view class="comment-label flex align-center">
                                ...
                                <view class="content flex align-center justify-between">
                                    <uni-rate :value="item.commentRank" color="#bfbfbf" :readonly="true"
                                        active-color="#ffa800" size="18" />
                            ...
                            <view class="comment-label flex" v-if="item.skuData && item.skuData.length > 0">
                                <view class="name">{{ $t("商品规格") }}:</view>
                                <view class="content flex-wrap">
                                    <view class="product-tag" v-for="skuItem in item.skuData">{{ skuItem.value }}</view>
                                </view>
                            </view>
                            <view class="comment-label flex">
                                <view class="name">{{ $t("评价标签") }}:</view>
                                <view class="content flex-wrap">
                                    <view class="product-tag" v-for="tag in item.commentTag">{{ tag }}</view>
                                </view>
                            </view>
                            <view class="comment-label flex">
                            ...
                            <view class="comment-label flex">
                                ...
                                <view class="content flex-wrap" v-if="item.showPics.length > 0">
                                    <view v-for="product in item.showPics" class="product-pic">
                                        <tig-image :src="product.picThumb" mode="aspectFill"
                                            @click="imagePreview(imageFormat(product.picUrl))"></tig-image>
                                    </view>
                                </view>
                            </view>
                        </view>
            ...
            <view class="empty-box" v-if="filterState.length == 0">
                <view class="pictrue">
                    <image :src="staticResource('common/data_empty.png')"></image>
                </view>
                <view class="txt">{{ $t("暂无评价") }}</view>
            </view>
            <loading-box v-model="loaded" :page="filterParams.page" :length="filterState.length"></loading-box>
        </view>
    </tig-layout>
</template>
