<template>
    <tig-layout ref="layoutRef" @loginSuccess="loginSuccess">
        <tig-navbar :showLeft="false" title="会员中心" background="linear-gradient(90deg, #fcf6fc, #f3f3fe)"></tig-navbar>
        <view class="user">
            <view class="user_header user_bg_color">
                <view class="picTxt">
                    <template v-if="!hasToken">
                        <view class="pictrue" @click="handleLogin">
                            <image class="pic-btn" :src="staticResource('common/empty-img-bg3.png')"> </image>
                        </view>
                    </template>
                    <template v-else>
                        <tig-upload class="pictrue" @change="userStore.getUserInfo()" requestUrl="user/user/modify_avatar">
                            <tig-image class="pic-btn" :src="imageFormat(member!.avatar)"> </tig-image>
                        </tig-upload>
                    </template>
                    <view class="text">
                        <template v-if="!hasToken">
                            <view @click="handleLogin" class="name">{{ $t("登录/注册") }}</view>
                        </template>
                        <template v-else>
        ...
        <recommend></recommend>
        <view class="shop-company">{{ configStore.shopCompany }}</view>
    </tig-layout>
