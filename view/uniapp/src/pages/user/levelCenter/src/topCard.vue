<template>
    <view class="top_card_container">
        <view class="margin">
            <swiper next-margin="16rpx" class="card_swiper" @animationfinish="handleAnimationFinish" :current="currentIndex">
                <template v-for="(item, index) in list" :key="item?.rankId">
                    <!-- 颜色背景 -->
                    <swiper-item class="card_swiper_item" v-if="item?.rankCardType === 1">
                        <view class="card_item" :class="item?.rankIco">
                            ...
                            <view class="card_badge" v-if="item?.rankLevel === userInfo.rankLevel">{{ $t("当前等级") }}</view>
                            <!-- <view class="card_code_icon" v-if="item?.rank_level === userInfo.rank_level"></view> -->
                            <view class="card_bottom">
                                <!-- 成长值模式 -->
                                <template v-if="userInfo.rankType === 1">
                                    <view class="level_value_wapper" v-if="item?.rankLevel === userInfo.rankLevel">
                                        <text class="level_value">{{ userInfo?.growth }}</text>
                                        <text class="desc">{{ $t("成长值") }}</text>
                                    </view>
                                    <view class="upgrades_wapper" v-else>
                    ...
                    <!-- 自定义背图片景模式 -->
                    <swiper-item class="card_swiper_item" v-if="item?.rankCardType === 2">
                        <view class="card_item" :style="`background-image: url(${item?.rankBg});`">
                            ...
                            <view class="card_badge" v-if="item?.rankLevel === userInfo.rankLevel">{{ $t("当前等级") }}</view>
                            <view class="card_code_icon" v-if="item?.rankLevel === userInfo.rankLevel"></view>
                            <view class="card_bottom">
                                <!-- 成长值模式 -->
                                <template v-if="userInfo.rankType === 1">
                                    <view class="level_value_wapper" v-if="item?.rankLevel === userInfo.rankLevel">
        ...
        <view class="level_rights_list">
            <template v-for="(item, index) in currentRightUnion.slice(0, 4)" :key="index">
                <!-- 固定权益 -->
                <view class="right_item" v-if="item.iconImg" @click="toRightDetail">
                    <view class="icon_img" :class="item.iconImg"></view>
                    <view class="name">{{ item.rightName }}</view>
                </view>
                <!-- 自定义权益 -->
                <view class="right_item" v-else-if="item.isChecked === 1" @click="toRightDetail">
            ...
            <!-- 权益长度超过4个 -->
            <view class="right_item" v-if="currentRightUnion.length > 4" @click="toRightDetail">
