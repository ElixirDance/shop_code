    ...
    <tig-layout title="我的订单">
        <view class="order">
            <view class="order-menu badgecolor">
                <up-tabs
                    lineColor="var(--general)"
                    :list="menuTypeList"
                    :activeStyle="{ padding: '0 10rpx' }"
                    :inactiveStyle="{ padding: '0 10rpx' }"
                    :scrollable="true"
                    @change="changeMenu"
                    keyName="value"
                ...
                <template v-for="item in orderList" :key="item.orderId">
                    <view class="order-list-item">
                        <view class="order-list-item-header">
                            <text class="special-text" v-if="item.shop" @click.stop="handleToShop(item.shop?.shopId)">
                            ...
                            <view class="item-content-pay">
                                <view class="item-content-pay-box">
                                    ...
                                    <view class="pay-item total-money"
                                        >{{ $t("总额") }}：<format-price :showText="false" :isBottom="false" :priceData="item.totalAmount"></format-price
                                    ></view>
                                    <view class="pay-item" v-if="item.orderStatus === 0 && Number(item.unpaidAmount) > 0">
                                        {{ $t("应付") }}：<format-price :showText="false" :isBottom="false" :priceData="item.unpaidAmount"></format-price>
                                    </view>
                                </view>
                            </view>
                        </view>
            ...
            <loading-box v-model="isLoadMore" :page="filterParams.page" :length="orderList.length"></loading-box>
            <emptyBox v-if="!isLoading && total === 0" mode="list" background="#f5f5f5" :text="$t('暂无订单')"></emptyBox>
        </view>
        <tig-loadingpage v-model="isLoading"></tig-loadingpage>
    </tig-layout>
</template>
