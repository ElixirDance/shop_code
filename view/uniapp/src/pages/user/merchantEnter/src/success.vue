<template>
    <view class="success-box">
        <image class="success-img" mode="heightFix" :src="staticResource('user/success.png')"></image>
        <view class="success-text">{{ $t("审核已通过") }}</view>
        <view class="success-btn">
            <tig-button
                :customStyle="{ borderColor: '#C1CCDE', color: '#013148', marginRight: '40rpx', height: '70rpx', width: '260rpx' }"
                plain
                @click="handleLink"
                >{{ $t("返回首页") }}</tig-button
            >
            <tig-button :customStyle="{ height: '70rpx', width: '260rpx' }" @click="handleLink('admin')">{{ $t("商家后台") }}</tig-button>
        </view>
        <view class="tip" v-if="initialUserInfo && initialUserInfo.initialPassword && initialUserInfo.mobile">
            {{
                isOverseas()
