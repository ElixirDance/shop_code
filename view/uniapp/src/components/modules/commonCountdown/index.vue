    ...
    <div class="countdown-box">
        <template v-if="countdownType === 'dailyCycle'">
            <template v-if="Array.isArray(props.dailyTime) && new Date().getTime() < Number(dailyTime[0])">
                <up-count-down @change="onChange" :format="format" :time="Number(timeInfo.dailyStarTime) - new Date().getTime()">
            ...
            <template v-else>
                <up-count-down @change="onChange" :format="format" :time="Number(timeInfo.dailyEndTime) - new Date().getTime()">
                    <template v-for="(item, index) in arr" :key="item">
                        <text class="time-item" :style="timeStyle">
                            {{ formatTime(timeData[timeDateMap[item]])
                            }}<template v-if="!zh">
        ...
        <template v-else>
            <up-count-down @change="onChange" :format="format" :time="Number(specifyTime) - new Date().getTime()">
                <template v-for="(item, index) in arr" :key="item">
                    <text class="time-item" :style="timeStyle">
                        {{ formatTime(timeData[timeDateMap[item]])
                        }}<template v-if="!zh">
