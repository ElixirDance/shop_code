<template>
    <tig-popup :zIndex="1000" v-model:show="show" @close="close">
        <template v-if="show">
            <view class="wechart-login-content">
                <view class="wechart-login-box">
                    ...
                    <template v-if="isChecked">
                        <tig-button
                            :customStyle="{ 'border-radius': '20rpx', height: '95rpx' }"
                    ...
                    <template v-else>
                        <tig-button :customStyle="{ 'border-radius': '20rpx', height: '95rpx' }" shape="square" @click="mobileLogin">手机号快捷登入</tig-button>
                    </template>
                    <!-- #endif -->
                    <!-- #ifndef MP-WEIXIN -->
                    <template v-if="isChecked">
                        <tig-button :customStyle="{ 'border-radius': '20rpx', height: '95rpx' }" shape="square" :loading="loginLoading" @click="wechatLogin">{{
                            $t("微信授权")
                        }}</tig-button>
                    </template>
                    <template v-else>
                        <tig-button :customStyle="{ 'border-radius': '20rpx', height: '95rpx' }" shape="square" @click="mobileLogin">{{
                            $t("微信授权")
                        }}</tig-button>
                    </template>
                    <!-- #endif -->
...
const updateUserOpenId = async () => {
    wx.login({
        success: async (res: any) => {
            const result = await setMiniProgramOpenid({
                code: res.code
            });
        },
        fail: (err: any) => {
