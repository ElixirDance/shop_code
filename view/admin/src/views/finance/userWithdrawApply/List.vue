                    ...
                    <a-spin :spinning="loading">
                        <el-table :data="filterState" :loading="loading" :total="total" row-key="id"
                            ...
                            <el-table-column label="提现账号类型" >
                                <template #default="{ row }">
                                    {{ row.accountData.accountType===1?'银行卡':(row.accountData.accountType===2?'支付宝':(row.accountData.accountType===3?'微信':'N/A')) }}
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column :width="150" fixed="right" label="操作">
                                <template #default="{ row }">
                                    ...
                                    <DeleteRecord :params="{ id: row.id }" :requestApi="delUserWithdrawApply"
                                                  @afterDelete="loadFilter" :disabled="row.status == 0">删除
                                    </DeleteRecord>
                                </template>
                            </el-table-column>
                            <template #empty>
