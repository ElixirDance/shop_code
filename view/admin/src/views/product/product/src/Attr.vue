<template>
    <div class="attr-form">
        <el-form-item label="属性模板" prop="type" class="inner-item">
            <el-space>
                <el-select v-model="attrTplId" placeholder="请选择" clearable :disabled="disabled">
        ...
        <el-form-item prop="brandLogo" label="商品属性">
            <div class="attr-list other-attr">
                <div class="attr-warp-title flex flex-align-center mb10" v-for="(item, index) in attrList.normal">
                    ...
                    <div class="ml10">
                        <el-space class="attr-row" v-for="(attr, idx) in item.attrList">
                            <TigInput v-model="attr.attrValue" placeholder="" width="260px" :disabled="disabled" />
                            <div v-if="0 && !disabled" class="iconfont icon-cha1 btn-remove" @click="removeAttr('normal', index, idx)"></div>
                        </el-space>
                    </div>
                </div>
            ...
            <template #label>
                <el-tooltip class="box-item" effect="light" placement="bottom">
                    ...
                    <div class="flex flex-align-center">
                        <div>附加规格</div>
                        <el-icon style="margin-left: 5px" size="14"><QuestionFilled /></el-icon>
                    </div>
                </el-tooltip>
            </template>
            <div class="attr-list other-attr">
                <div class="attr-warp-title mb10" v-for="(item, index) in attrList.extra">
                    <div class="flex">
                        <div class="title flex">
                            <div v-if="!disabled" class="btn-remove3" style="margin-top: 8px" @click="removeAttrList('extra', index)">
                                <i class="iconfont icon-cha1"></i>
                            </div>
                            <div class="label">
                                {{ item.attrName }}
                        ...
                        <div class="ml10">
                            <el-space class="attr-row mb10" v-for="(attr, idx) in item.attrList" :size="16">
                                <TigInput v-model="attr.attrValue" placeholder="" width="180px" :disabled="disabled" />
                                <el-space :size="0">
                                    附加金额：
                                    <TigInput type="decimal" :disabled="disabled" v-model="attr.attrPrice" width="100px" />
                                </el-space>
                    ...
                    <el-space class="attr-row mt10" style="margin-left: 70px">
            ...
            <el-form-item label="规格明细" v-if="productForm.length > 0" prop="productList">
                <div class="lyecs-form-table">
                    <el-table :data="productList" style="width: 100%">
                        ...
                        <el-table-column prop="skuPrice" label="一口价" width="150" fixed="right">
                            ...
                            <template #default="{ row, $index }">
                                <el-form-item label="" :prop="`productList.${$index}.skuPrice`" :rules="[{ required: true, validator: validatePrice }]">
                                    <PriceInput v-model:modelValue="row.skuPrice" :disabled="disabled" :slot="false"></PriceInput>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column prop="skuSn" label="商品货号" width="150" fixed="right">
                            <template #header>
                                <PopForm
                                    :isDirect="true"
                                    :isHover="false"
                                    ...
                                    label="商品货号"
                                >
                                    <div class="flex flex-align-center flex-justify-center">商品货号</div>
                                </PopForm>
                            </template>
                            <template #default="{ row, $index }">
