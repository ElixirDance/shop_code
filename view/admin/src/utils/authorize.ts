    ...
    routes.forEach(route => {
        if (route.meta.title && route.meta.title !== 'undefined') {
            if (!isAuthorized() || licensedData.deCopyright == 0) {
                const string = licensedData.poweredBy || ' - powered by tigshop';
                if (!route.meta.title?.includes(string)) {
                    route.meta.title += string;
                }
            }
            if (isAuthorized() && licensedData.deCopyright != 0 && licensedData.poweredByStatus == 0) {
                if (!route.meta.title?.includes(licensedData.poweredBy)) {
                    route.meta.title += licensedData.poweredBy || '';
                }
            }
        }
        if (route.children) {
...
export function removeSuffix(title: string): string {
    // 定义要去掉的后缀
    const suffix = licensedData.poweredBy || ' - powered by tigshop';
    // 检查标题是否包含后缀，如果包含则去掉
...
export function isShowAvatar() {
    ...
    if (licensedData.isEnterprise == 0) {
        if (licensedData.licensedType.includes('pro')) {
